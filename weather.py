...
def weather(time):
    ...
    pp = data1.findAll('span',{'class':'indicator'})
    e = ''
    r = ''
    check = 0
    ...
    if time == '내일':
        return '현재 위치는 ' + find_address + '입니다... 오전에는 ' + r1[1].text + '이고 오후는 ' + r1[2].text + '입니다.' + '오전 온도는 ' + e1[1].text + '도. 오후 온도는 ' + e1[2].text + '도 입니다 그리고 오전 미세먼지는' +pp[1].text +'오후 미세먼지는' + pp[2].text + '입니다'
    if time == '모레':
        return '현재 위치는 ' + find_address + '입니다... 오전에는 ' + r1[3].text + '이고 오후는 ' + r1[4].text + '입니다.' + '오전 온도는 ' +e1[3].text + '도. 오후 온도는 ' + e1[4].text + '도 입니다 그리고 오전 미세먼지는'+pp[3].text +'오후 미세먼지는' + pp[4].text + '입니다'
